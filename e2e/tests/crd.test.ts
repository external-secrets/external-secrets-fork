...
const { customResourceManagerDisabled } = config

describe('CRD', () => {
  it('ensure CRD is managed correctly', async () => {
    const res = await kubeClient.apis['apiextensions.k8s.io'].v1beta1
    ...
    expect(
      res.body.metadata.annotations['app.kubernetes.io/managed-by']
    ).to.equal(managedBy)
  })

  it('should reject invalid ExternalSecret manifests', async () => {
    kubeClient.apis[customResourceManifest.spec.group].v1
      .namespaces('default')
      ...
      .catch((err: any) => expect(err).to.be.an('error'))
  })
})
