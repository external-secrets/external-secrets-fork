...
import { waitForSecret, uuid, delay } from './framework'
...
describe('secretsmanager', async () => {
  it('should pull existing secret from secretsmanager and create a secret with its values', async () => {
    let result: any = await secretsmanager
      .createSecret({
  ...
  it('should pull TLS secret from secretsmanager', async () => {
    ...
    const secret = await waitForSecret(
      'default',
      `e2e-secretmanager-tls-${uuid}`
    )
    expect(secret).to.not.equal(undefined)
  ...
  describe('permitted annotation', async () => {
    ...
    describe('assuming role', async () => {
      it('should not pull from secretsmanager', async () => {
        ...
        const secret = await waitForSecret(
          'default',
          `e2e-secretmanager-permitted-tls-${uuid}`
        )
    ...
    describe('enforcing naming convention', async () => {
      it('should not pull from secretsmanager', async () => {
        ...
        const secret = await waitForSecret(
          'default',
          `e2e-secretmanager-permitted-key-${uuid}`
        )
        expect(secret).to.equal(undefined)
        ...
        expect(result).to.not.equal(undefined)
        expect(result.body.status.status).to.contain(
          `key name e2e/${uuid}/another_credentials does not match naming convention /permitted/path/.*`
        )
      })
    })
  })
