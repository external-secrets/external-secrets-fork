...
describe('Poller', () => {
  ...
  let kubeNamespaceMock: any
  let externalSecretsApiMock: any
  let fakeExternalSecret: any
  ...
  const namingPermittedAnnotation =
  ...
  beforeEach(() => {
    ...
    kubeNamespaceMock.externalsecrets = sinon
      .stub()
      .returns(externalSecretsApiMock)
    ...
    ].v1.namespaces = sinon.stub().returns(kubeNamespaceMock)
  ...
  describe('_createSecretManifest', () => {
    ...
    it('creates secret manifest - no type (backwards compat)', async () => {
      ...
      expect(
        backendMock.getSecretManifestData.calledWith({
          spec: {
            backendType: 'fakeBackendType',
            name: 'fakeSecretName',
            properties: ['fakePropertyName1', 'fakePropertyName2']
    ...
    it('creates secret manifest - with type (backwards compat)', async () => {
      const poller = pollerFactory({
        ...
        backendType: 'fakeBackendType',
        name: 'fakeSecretName',
        data: ['fakePropertyName1', 'fakePropertyName2']
      })
  ...
  describe('_poll', () => {
    ...
    it('polls secrets', async () => {
      ...
      expect(
        loggerMock.info.calledWith(
          `running poll on the secret ${poller._namespace}/${poller._name}`
        )
      ).to.equal(true)
    ...
    it('logs error if storing secret operation fails', async () => {
      ...
      expect(
        poller._updateStatus.calledWith(`ERROR, ${error.message}`)
      ).to.equal(true)
      expect(
        loggerMock.error.calledWith(
          error,
          `failure while polling the secret ${poller._namespace}/${poller._name}`
        )
      ).to.equal(true)
    })
  })
  ...
  describe('_updateStatus', () => {
    it('handles 409 - Conflict', async () => {
      ...
      expect(
        loggerMock.info.calledWith(
          `status update failed for externalsecret ${poller._namespace}/${poller._name}, due to modification, new poller should start`
        )
      ).to.equal(true)
    })
    ...
    it('handles odd whitespace and newlines in status', async () => {
      ...
      await poller._updateStatus('\n\n\nLots      of spaces\n\n\n').then(() => {
        const statusMessage = externalSecretsApiMock.status.put.getCall(0)
          .args[0].body.status.status
  ...
  describe('_scheduleNextPoll', () => {
    ...
    describe('last sync', () => {
      ...
      it('with new last sync - queues poll', async () => {
        ...
        expect(externalSecretsApiMock.status.get.calledWith()).to.equal(true)
        expect(
          poller._setNextPoll.calledWith(
            poller._intervalMilliseconds - elapsedTime
          )
        ).to.equal(true)
      })
    ...
    describe('generation', () => {
      ...
      it('with newer observed generation - queues poll', async () => {
        ...
        expect(
          poller._setNextPoll.calledWith(
            fakeDate.getTime() - (Date.now() - poller._intervalMilliseconds)
          )
        ).to.equal(true)
      })
    ...
    it('logs error if it fails', async () => {
      ...
      expect(
        loggerMock.error.calledWith(
          error,
          'status check went boom for fakeNamespace/fakeSecretName'
        )
      ).to.equal(true)
    })
  })
  ...
  describe('_upsertKubernetesSecret', () => {
    ...
    it('creates new secret', async () => {
      ...
      expect(
        kubeNamespaceMock.secrets.post.calledWith({
          body: {
            apiVersion: 'v1',
            kind: 'Secret',
            metadata: {
    ...
    it('updates secret', async () => {
      ...
      expect(kubeNamespaceMock.secrets.calledWith('fakeSecretName')).to.equal(
        true
      )

      expect(
        kubeNamespaceMock.put.calledWith({
          body: {
            apiVersion: 'v1',
            kind: 'Secret',
            metadata: {
    ...
    it('does not permit update of secret', async () => {
      ...
      expect(error.message).equals(
        'not allowed to fetch secret: fakeNamespace/fakeSecretName: namespace does not allow to assume role arn:aws:iam::123456789012:role/test-role'
      )
    })

    it('fails storing secret', async () => {
      const internalErrorServer: any = new Error('Internal Error Server')
  ...
  describe('start', () => {
    ...
    it('starts poller', async () => {
      ...
      expect(
        loggerMock.info.calledWith(
          `starting poller for ${poller._namespace}/${poller._name}`
        )
      ).to.equal(true)
  ...
  describe('stop', () => {
    ...
    it('stops poller', async () => {
      ...
      expect(
        loggerMock.info.calledWith(
          `stopping poller for ${poller._namespace}/${poller._name}`
        )
      ).to.equal(true)
      expect(poller._timeoutId).to.equal(null)
    })
    ...
    it('should restrict access to certain roles per namespace ', () => {
      const testcases = [
        ...
        },
        {
          // test regex
          ns: {
            metadata: { annotations: { [rolePermittedAnnotation]: '.*' } }
          },
        ...
        },
        {
          ...
          ns: {
            metadata: {
              annotations: {
                [rolePermittedAnnotation]: 'arn:aws:iam::123456789012:role/.*'
              }
            }
          },
        ...
        },
        {
          ...
          ns: {
            metadata: {
              annotations: { [rolePermittedAnnotation]: 'my-kiam-role.*' }
            }
          },
          descriptor: {},
          permitted: true
    ...
    it('should restrict access as defined in namespace naming convention ', () => {
      const testcases = [
        ...
        },
        {
          // empty annotation
          ns: {
            metadata: { annotations: { [namingPermittedAnnotation]: '' } }
          },
          descriptor: {},
          permitted: true
        },
        {
          ...
          descriptor: {
            data: [{ key: 'whatever', name: 'somethingelse' }]
          },
          permitted: true
        },
        {
          ...
          ns: {
            metadata: {
              annotations: { [namingPermittedAnnotation]: 'dev/team-a/.*' }
            }
          },
          descriptor: {
            data: [{ key: 'dev/team-a/secret', name: 'somethingelse' }]
          },
          permitted: true
        },
        {
          ...
          descriptor: {
            data: [{ key: 'dev/team-b/secret', name: 'somethingelse' }]
          },
          permitted: false
        },
        {
        ...
        },
        {
          ...
          descriptor: {
            data: [{ key: 'dev/team-a/ok-secret', name: 'somethingelse' }],
            dataFrom: ['dev/team-b/bad-secret']
          },
          permitted: false
        },
        {
          ...
          ns: {
            metadata: {
              annotations: {
                [namingPermittedAnnotation]: ['dev/team-a/.*', 'common/.*']
              }
            }
          },
          descriptor: {
        ...
        },
        {
          ...
          ns: {
            metadata: {
              annotations: {
                [namingPermittedAnnotation]: ['dev/team-b/.*', 'common/.*']
              }
            }
          },
          descriptor: {
            data: [{ key: 'common/generic-secret', name: 'genericsecret' }],
            dataFrom: ['common/generic-secret']
          },
          permitted: true
        },
        {
      ...
      for (let i = 0; i < testcases.length; i++) {
        const testcase = testcases[i]
        const verdict = poller._isPermitted(testcase.ns, testcase.descriptor)
        expect(verdict.allowed, `test case ${i + 1}`).to.equal(
          testcase.permitted
        )
      }
    })
    ...
    it('should enforce namespace annotations`', () => {
      const testcases = [
        ...
        },
        {
          ...
          ns: {
            metadata: { annotations: { [namingPermittedAnnotation]: 'a' } }
          },
          descriptor: {
            dataFrom: ['a'],
            roleArn: 'a'
