...
import { CustomResourceManager } from './custom-resource-manager'

describe('CustomResourceManager', () => {
  let customResourceManager: any
  let fakeCustomResource: any
  ...
  describe('_createResource', () => {
    it('creates custom resource', async () => {
      ...
      expect(
        v1beta1Mock.customresourcedefinitions.post.calledWith({
  ...
  describe('_getResource', () => {
    it('gets custom resource', async () => {
      v1beta1Mock.customresourcedefinitions.returns(
        v1beta1Mock.customresourcedefinitions
      )
      v1beta1Mock.customresourcedefinitions.get = sinon
        .stub()
      ...
      expect(
        v1beta1Mock.customresourcedefinitions.calledWith(fakeResourceName)
      ).to.equal(true)
  ...
  describe('_updateResource', () => {
    it('updates custom resource', async () => {
      ...
      const fakeResourceVersion =
        fakeCustomResource.body.metadata.resourceVersion
      ...
      expect(
        v1beta1Mock.customresourcedefinitions.put.calledWith({
          body: fakeCustomResourceManifest
        })
      ).to.equal(true)
    })
  ...
  describe('_upsertResource', () => {
    ...
    it('creates custom resource', async () => {
      ...
      expect(
        customResourceManager._createResource.calledWith({
          customResourceManifest: fakeCustomResourceManifest
        })
      ).to.equal(true)
    ...
    it('updates custom resource', async () => {
      const conflictError: any = new Error('fake conflict error')
      ...
      expect(
        customResourceManager._getResource.calledWith({
          resourceName: fakeResourceName
        })
      ).to.equal(true)
      expect(
        customResourceManager._updateResource.calledWith({
          customResource: fakeCustomResource,
          customResourceManifest: fakeCustomResourceManifest
        })
      ).to.equal(true)
  ...
  describe('managerCrd', () => {
    ...
    it('upsert when enabled', async () => {
      ...
      expect(
        customResourceManager._upsertResource.calledWith({
          customResourceManifest: fakeCustomResourceManifest
        })
      ).to.equal(true)
      expect(
        kubeClientMock.addCustomResourceDefinition.calledWith(
          fakeCustomResourceManifest
        )
      ).to.equal(true)
    })
    ...
    it('do nothing when disabled', async () => {
      ...
      const disabledMsg =
        'Custom resource manager has been disabled, CRD will not be managed.'
