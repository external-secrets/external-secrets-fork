...
import { Data, DataFrom } from './backends/abstract-backend'
...
export class Poller {
  ...
  private _intervalMilliseconds: any
  ...
  private _externalSecret: any
  readonly _spec: any

  private _ownerReference: any
  ...
  constructor({
    ...
    this._status = this._kubeClient.apis[
      this._customResourceManifest.spec.group
    ].v1
      .namespaces(this._namespace)
      [this._customResourceManifest.spec.names.plural](this._name).status
  }
  ...
  async _createSecretManifest() {
    ...
    const data = await this._backends[spec.backendType].getSecretManifestData({
      spec
    })
    ...
    const secretManifest = {
      ...
      metadata: {
        name: this._name,
        ownerReferences: [this._ownerReference]
      },
      type,
      data
    }
  ...
  async _poll() {
    this._logger.info(
      `running poll on the secret ${this._namespace}/${this._name}`
    )
    ...
    try {
      ...
      this._logger.error(
        err,
        `failure while polling the secret ${this._namespace}/${this._name}`
      )
  ...
  async _upsertKubernetesSecret() {
    ...
    if (!verdict.allowed) {
      throw new Error(
        `not allowed to fetch secret: ${this._namespace}/${this._name}: ${verdict.reason}`
      )
    }
  ...
  async _updateStatus(status: any) {
    // Remove newlines, spaces, and tabs from status string
    status = status.replace(/(\r\n|\n|\r| (?= ))/gm, '').replace(/\t/g, ' ')

    try {
      this._logger.debug(
        `updating status for ${this._namespace}/${this._name} to: ${status}`
      )
      ...
      if (err.statusCode !== 409) {
        this._logger.error(
          err,
          `failure while updating status for externalsecret ${this._namespace}/${this._name}`
        )
        throw err
      }
      this._logger.info(
   ...
   */
  _isPermitted(namespace: any, descriptor: any) {
    let allowed = true
    let reason = ''

    if (!namespace.metadata.annotations) {
      allowed = !this._enforceNamespaceAnnotation
      reason = this._enforceNamespaceAnnotation
        ? 'Namespace annotation is required'
        : ''
      return {
        allowed,
        reason
    ...
    const externalData: Data = descriptor.data || descriptor.properties
    const namingConvention =
      namespace.metadata.annotations[this._namingPermittedAnnotation]
    let reNaming: RegExp
    ...
    if (namingConvention && externalData) {
      externalData.forEach((secretProperty) => {
    ...
    const externalDataFrom: DataFrom = descriptor.dataFrom
    if (namingConvention && externalDataFrom) {
      externalDataFrom.forEach((secretProperty) => {
    ...
    if (
      !namespace.metadata.annotations[this._rolePermittedAnnotation] &&
      this._enforceNamespaceAnnotation
    ) {
      allowed = false
  ...
  async _scheduleNextPoll() {
    try {
      const {
        body: { status: { lastSync = null, observedGeneration = 0 } = {} } = {}
      ...
      this._logger.error(
        err,
        `status check went boom for ${this._namespace}/${this._name}`
      )
    }
  }
  ...
  _setNextPoll(nextPollIn = this._intervalMilliseconds) {
    ...
    this._logger.debug(
      `next poll for ${this._namespace}/${this._name} in ${nextPollIn} ms`
    )
  }
