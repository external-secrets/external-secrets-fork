...
export class CustomResourceManager {
   ...
   */
  constructor({ kubeClient, logger, disabled }: any) {
   ...
   */
  _createResource({ customResourceManifest }: any) {
    return this._kubeClient.apis[
      'apiextensions.k8s.io'
    ].v1beta1.customresourcedefinitions.post({ body: customResourceManifest })
  }
   ...
   */
  _getResource({ resourceName }: any) {
    return this._kubeClient.apis['apiextensions.k8s.io'].v1beta1
   ...
   * @returns {Promise} Promise object representing operation result.
   */
  _updateResource({ customResource, customResourceManifest }: any) {
   ...
   * @returns {Promise} Promise object representing operation result.
   */
  async _upsertResource({ customResourceManifest }: any) {
    ...
    for (let attempt = 0; attempt <= 5; attempt++) {
      try {
        ...
        return await this._updateResource({
          customResource,
  ...
  async manageCrd({ customResourceManifest }: any) {
