...
import { KeyOptions, SpecOptions } from './abstract-backend'

/** Vault backend class. */
export class VaultBackend extends KVBackend {
  private _client: any
  private _tokenRenewThreshold: any
  private _serviceAccountToken: any
   ...
   */
  constructor({ client, tokenRenewThreshold, logger }: any) {
    super({ logger })
    this._client = client
  ...
  _fetchServiceAccountToken() {
    if (!this._serviceAccountToken) {
      this._serviceAccountToken = fs.readFileSync(
  ...
  async _get({
    key,
    specOptions: { vaultMountPoint, vaultRole, kvVersion = 2 }
  }: {
    key: string
    ...
    if (!this._client.token) {
      ...
      this._logger.debug(
        `vault token valid for ${tokenStatus.data.ttl} seconds, renews at ${this._tokenRenewThreshold}`
      )
