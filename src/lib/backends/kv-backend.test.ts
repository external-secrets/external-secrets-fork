...
import { KVBackend as AbstractKVBackend } from './kv-backend'

class KVBackend extends AbstractKVBackend {
  _get(): Promise<string> {
...
describe('kv-backend', () => {
  ...
  describe('_fetchDataValues', () => {
    ...
    it('handles invalid JSON objects', async () => {
      ...
      expect(secretPropertyValues).to.deep.equal([{}])
    })
    ...
    it('fetches secret property with version', async () => {
      ...
      expect(secretPropertyValues).deep.equals([
        { fakePropertyName1: 'fakePropertyValue1' },
        { fakePropertyName2: 'fakePropertyValue2' },
        { fakePropertyName3: 'fakePropertyValue3' }
      ])
    })
  })
  ...
  describe('_fetchDataFromValues', () => {
    ...
    it('fetches secret property values using the specified options', async () => {
      kvBackend._get
        .onFirstCall()
        .resolves('{"fakePropertyName1":"fakePropertyValue1"}')
      kvBackend._get
        .onSecondCall()
        .resolves('{"fakePropertyName2":"fakePropertyValue2"}')
      ...
      expect(dataFromValues).deep.equals([
        { fakePropertyName1: 'fakePropertyValue1' },
        { fakePropertyName2: 'fakePropertyValue2' }
      ])
    })
  ...
  describe('getSecretManifestData', () => {
    ...
    it('returns secret manifest data', async () => {
      kvBackend._fetchDataValues.resolves([
        {
      ...
      kvBackend._fetchDataFromValues.resolves([
        {
    ...
    it('handles undefined data', async () => {
      ...
      const manifestData = await kvBackend.getSecretManifestData({ spec: {} })
    ...
    it('makes correct calls - with data and role', async () => {
      ...
      expect(
        kvBackend._fetchDataValues.calledWith({
          data: [
            {
              key: 'fakePropertyKey1',
              name: 'fakePropertyName1'
      ...
      expect(
        kvBackend._fetchDataFromValues.calledWith({
          dataFrom: [],
          specOptions: { roleArn: 'my-role' }
        })
      ).to.equal(true)
