...
import { SecretsManagerBackend } from './secrets-manager-backend'

describe('SecretsManagerBackend', () => {
  let clientMock: any
  let loggerMock: any
  let clientFactoryMock: any
  let assumeRoleMock: any
  let secretsManagerBackend: any
  const specOptions = {}
  const keyOptions = {}
  ...
  describe('_get', () => {
    ...
    it('returns secret property value', async () => {
      ...
      expect(
        clientMock.getSecretValue.calledWith({
    ...
    it('returns secret property value with versionStage', async () => {
      ...
      expect(clientFactoryMock.getCall(0).args).deep.equals([])
      expect(assumeRoleMock.callCount).equals(0)
      expect(secretPropertyValue).equals(
        'fakeSecretPropertyValuePreviousVersion'
      )
    })
