...
import { VaultBackend } from './vault-backend'
const logger = pino({
  serializers: {
...
describe('VaultBackend', () => {
  ...
  const quotedSecretValueAsBase64 = Buffer.from(quotedSecretValue).toString(
    'base64'
  )
  ...
  describe('_get', () => {
    beforeEach(() => {
      ...
      clientMock.tokenLookupSelf = sinon
        .stub()
        .returns(mockTokenLookupResultMustRenew)
    ...
    it('returns secret property value if a token exists that does not need renewal', async () => {
      ...
      clientMock.tokenLookupSelf = sinon
        .stub()
        .returns(mockTokenLookupResultNoRenew)
  ...
  describe('getSecretManifestData', () => {
    beforeEach(() => {
      ...
      clientMock.tokenLookupSelf = sinon
        ...
        .returns(mockTokenLookupResultMustRenew)
      clientMock.tokenRenewSelf = sinon.stub().returns(true)
      clientMock.kubernetesLogin = sinon
        .stub()
        .returns({ auth: { client_token: '1234' } })
