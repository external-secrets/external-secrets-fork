...
export class GCPSecretsManagerBackend extends KVBackend {
  private _client: any
   ...
   */
  async _get({ key, keyOptions, specOptions: { projectId } }: GetSecret) {
    if (!projectId) {
    ...
    this._logger.info(
      `fetching secret ${key} from GCP Secret for project ${projectId} with version ${secretVersion}`
    )
